namespace Company.PL.HelperImage
{
    public static class DecumentSettings
    {
        ...
        public static string UploadFile(IFormFile file, string flodername)
        {
            ...
            var fileStream = new FileStream(FullPathImage, FileMode.Create);
            file.CopyTo(fileStream);
            return fileName;

        }
        // 2- delete image
        public static void DeleteFile(string fileName, string flodername)
        {
            // 1- get folder path
            var FullPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", flodername, fileName);
